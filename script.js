...
function maybeNotify(sym, sig){
  ...
  try{ new Notification(`${sig.label}: ${sym}`, { body:`${sig.tier} · ${sig.confidence}% confidence`, silent:true }); }catch(e){}
}
...
async function runScan(){
  ...
  try{
    ...
    setError(err && err.friendly ? err.friendly : 'Server Busy — please try again.');
  } finally {
...
document.getElementById('customAddBtn').addEventListener('click', ()=>{
  const v = document.getElementById('customSymbolInput').value.trim().toUpperCase();
  if(!v) return;
